...
/*********************************************************************
*
*       Chip specifics RENESAS RX and GNU compiler
*
**********************************************************************
*/
...
#define OS_SIZEOF_INT             (4)                        /* Size of integer in bytes*/
    ...
    #define OS_BIG_ENDIAN           1                        /* 1 => big endian*/
#else 
  #ifdef __RX_LITTLE_ENDIAN__
    #define OS_BIG_ENDIAN           0                        /* 0 => little endian*/
  #else
    #error "Please check your compiler settings. __RX_BIG_ENDIAN__ or __RX_LITTLE_ENDIAN__ has to be defined."
  #endif
#endif
...
#define OS_EnterRegion            OS_EnterRegionFunc         /* Use function instead of macro*/
...
#define TMR01_PLCK_DIV1			0		/*OSPSSYS=1*/
#define TMR01_PLCK_DIV2			1		/*OSPSSYS=2*/
#define TMR01_PLCK_DIV8			2		/*OSPSSYS=8*/
#define TMR01_PLCK_DIV32		3		/*OSPSSYS=32*/
#define TMR01_PLCK_DIV64		4		/*OSPSSYS=64*/

	#define OS_SYSTEM_PS			(TMR01_PLCK_DIV8) /* definire in OS_Config.h */

	#if		OS_SYSTEM_PS==TMR01_PLCK_DIV1
		#define   OS_PSSYS       		(1)
	#elif	OS_SYSTEM_PS==TMR01_PLCK_DIV2
		#define   OS_PSSYS       		(2)
	#elif	OS_SYSTEM_PS==TMR01_PLCK_DIV8
		#define   OS_PSSYS       		(8)
	#elif	OS_SYSTEM_PS==TMR01_PLCK_DIV32
		#define   OS_PSSYS       		(32)
	#elif	OS_SYSTEM_PS==TMR01_PLCK_DIV64
		#define   OS_PSSYS       		(64)
	#else
	  #error "Unsupported value for OS_SYSTEM_PS."
	#endif
#elif defined(OS_SYSTIMER_CMT0)
	#define CMT0_PLCK_DIV8			0
	#define CMT0_PLCK_DIV32			1
	#define CMT0_PLCK_DIV128		2
	#define CMT0_PLCK_DIV512		3

	#if		OS_SYSTEM_PS==CMT0_PLCK_DIV8
		#define   OS_PSSYS       		(8)
	#elif	OS_SYSTEM_PS==CMT0_PLCK_DIV32
		#define   OS_PSSYS       		(32)
	#elif	OS_SYSTEM_PS==CMT0_PLCK_DIV128
		#define   OS_PSSYS       		(128)
	#elif	OS_SYSTEM_PS==CMT0_PLCK_DIV32
		#define   OS_PSSYS       		(32)
	#elif	OS_SYSTEM_PS==CMT0_PLCK_DIV512
		#define   OS_PSSYS       		(512)
	#else
	  #error "Unsupported value for OS_SYSTEM_PS."
	#endif
#else
	#error "Unsupported value for OS_SYSTIMER_XXX."
#endif

#ifndef		OS_PCLK_ALM			/* Peripheral clock for system alarm   */
#define OS_PCLK_ALM       		(OS_FSYS/1)
#endif

#ifndef		OS_PCLK_TIMER		/* Peripheral clock for timer   */
#define	OS_PCLK_TIMER 			(OS_FSYS/1)
#endif
...
#define OS_ENABLE_INTS_SAVE_IPL()     \
  { unsigned int  Flg;                \
  __builtin_rx_mvtc(0, __builtin_rx_mvfc(0) | (1uL << 16)); \
  Flg = OS_Ipl_EI;                    \
  OS__SaveIPL(); {
...
#define OS_VERSION_GENERIC (38800u)       /*   Generic version 3.88 */
...
#define OS_LINK_MAILBOXES (OS_DEBUG)
  ...
  #define OS_INIT_EXPLICITLY  (1)           /* If enabled, OS_InitKern() initializes Non-Zero globals. Required only for faulty compiler (or startup-code) which do not perform proper init of globals during startup.*/
  	  	  	  	  	  	  	  	  	  	  	/*DO NOT CHANGE: Required by Extensions.*/
#endif
...
/**
\ingroup OS_EXTENSION
Called when a fatal error has occured and the kernel
can not continue. 
...
/**
Converts nanoseconds to System timer cycles
 */
#define OS_NANOSECS_TO_SYSTEMTIMERCYCLES(ns)		(((ns)/(1000000000/(OS_FSYS/OS_PSSYS))))
...
/**
\ingroup OS_EXTENSION
 When this happens, not critical error has occured, but the called API behaves 
 as non expected (e.g. object is in use).

 This routine can be modified to suit your needs ...
 E.g. -a red LED could light up. 
      -When using an  emulator, you may set a breakpoint here.
      -filter on args may be applied.
...
*/
void OS_X_Error(unsigned char ErrCode, unsigned char ServiceId);


#ifdef __OSEXTENSIONSGLOBAL_C__
  #define OS_X_EXTERN        /* Declare variables if included by RTOSKERN.C */
  #define OS_X_EXTERN_INIT(Var, Val) Var=Val
#else
  #define OS_X_EXTERN extern /* Define variables if included by anyone else */
  #define OS_X_EXTERN_INIT(Var, Val) Var
#endif


/**
\defgroup	OS_EXTENSION_ERRCODE Error Codes
@{
  Error codes
*/

#define OS_ERR_USER                           (512)         /**< Error Code base value */
/*@}*/
...
/**
@{
\ingroup   	OS_EXTENSION_ALM_MCF51AG128
*/
#ifndef   OS_ALARM_TICK
  #define OS_ALARM_TICK                 unsigned short int  /**< tick type  for alarm */
#endif

/*#define OS_ALARM_HW_CNT                 OS_TPMCNT*/
/*#define OS_ALARM_HW_CNT_TARGET_VAL      OS_ALMTPMCxV*/
/*#define OS_ALARM_HW_CNT_SC              OS_ALMTPMCxSC*/
...
/**
\defgroup	OS_EXTENSION_ALM_ERRCODE Error Codes
@{
\ingroup  OS_EXTENSION_ALARMS
Alarm module Error codes for OS_Error
*/
#define OS_ERR_INV_ALARM                      (OS_ERR_USER + 1) /**< Invalid alarm */
#define OS_ERR_2USE_ALARM                     (OS_ERR_USER + 2) /**< Alarm in use */

#define OS_ERR_OOR_CYCLES                     (OS_ERR_USER + 3) /**< cycles value is out of range */
#define OS_ERR_OOR_US                         (OS_ERR_USER + 4) /**< us value is out of range  */

#define OS_ERR_EMPTY_ALARMHOOK                (OS_ERR_USER + 5) /**< Invalid alarm hook routine */
/* @} */


#define OS_MAX_ALARMcyc                       ((0xFFFF*(OS_PCLK_ALM/OS_PSSYS) / 1000000))
#define OS_MAX_ALARMus                        (((0xFFFF*1000000.0) / (OS_PCLK_ALM/OS_PSSYS)))
/*
#if (OS_MAX_ALARMus == 25816)
#error "OS_MAX_ALARMus"
#endif
*/
/* assertion not out of range cycles */
#define OS_ASSERT_NOTOORALARMcyc(cycles)      OS_ASSERT((cycles < (OS_U32)OS_MAX_ALARMcyc), OS_ERR_OOR_CYCLES)
#define OS_ASSERT_NOTOORALARMus(us)           OS_ASSERT((us < (OS_U32)OS_MAX_ALARMus),      OS_ERR_OOR_US)


/**
\defgroup	OS_EXTENSION_ALM_ERRID Services and Error IDs
@{
  Services and Error IDs for OS_X_Error
  \ingroup   	OS_EXTENSION_ALARMS
*/
...
@{
*/
#define OS_ALARM_SERVICE_ID_START                             0 /**< Service id start */

#define OS_StartAlarm_ServiceId                               (OS_ALARM_SERVICE_ID_START + 1) /**< OS_StartAlarm Service Id */
#define OS_SetAlarmActivationsCount_ServiceId                 (OS_ALARM_SERVICE_ID_START + 2) /**< OS_SetAlarmActivationsCount Service Id*/
#define OS_SetAlarmOffset_ServiceId                           (OS_ALARM_SERVICE_ID_START + 3) /**< OS_SetAlarmOffset Service Id*/
#define OS_SetAlarmPeriod_ServiceId                           (OS_ALARM_SERVICE_ID_START + 4) /**< OS_SetAlarmPeriod Service Id*/
/* @} */
...
@{
*/
#define OS_ALARM_ERROR_ID_START                               0 /**< Extended error id*/
#define OS_AlarmActive_ErrorId                                (OS_ALARM_ERROR_ID_START + 1) /**< Alarm already active */
/* @} */
/* @} */
...
struct OS_alarm {
  OS_ALARM * pNext;                 /**/
  voidRoutine* Hook;                /* hook routine */
  OS_ALARM_TICK TimeTick;           /* time expiration */
  OS_ALARM_TICK StartTimeTick;      /* offset on start */
  OS_ALARM_TICK PeriodTick;         /* period */
  char    Active;                   /*  0 :   inactive
                                        1 :   active 
                                    */
  int    ActivationsCount;          /* counter before became inactive */
#if OS_DEBUG
  char Id;                      
#endif
};
...
/**
\defgroup	OS_EXTENSION_ALM_TYPES  Types
@{
  Types
  \ingroup   	OS_EXTENSION_ALARMS
*/
typedef void OS_ALARMROUTINE(void); /**< Alarm hook routine type*/
/* @} */
...
typedef struct OS_EXTENSION_GLOBAL {
  OS_ALARM     * pAlarm;            /* Linked list of all active Timers*/
} OS_EXTENSIONS_GLOBAL;
...
#if OS_COMPILER_LOCATION_LEFT
    extern    OS_SADDR OS_EXTENSIONS_GLOBAL       OS_Extensions_Global;
    OS_X_EXTERN OS_SADDR  volatile OS_U8  OS_X_Status;
#else
    extern    OS_EXTENSIONS_GLOBAL OS_SADDR       OS_Extensions_Global;
#endif    
...
#define OS_pAlarm                       OS_Extensions_Global.pAlarm
...
/* Routines in RTOSInit.c */
/**
\defgroup	OS_EXTENSION_ALM_MCF51AG128 MCF51 AG128 specific implementation
@{
\ingroup   	OS_EXTENSION_ALARMS
  High resolution Alarms uses one of the two channel of the same hw counter used by EmbOS in outout compare mode
  without toggle on the pin.
*/

/**
  Alarm handler
*/
void _OS_SystemAlarmHandler		(void) __attribute__ ((interrupt));

/**
  Writes the new value for the output compare
*/
void OS_AlarmCounterArm(OS_ALARM_TICK Tick);
/**
  Returns the hw counter value
*/
...
/**
  Returns if alarm is raised
*/
int OS_AlarmRaised(void);
/**
  Disables output compare
*/
...
**********************************************************************
*/
void OS_CreateAlarm_(OS_ALARM* pAlarm, OS_ALARMROUTINE* Hook, OS_U32 usPeriod, OS_U32 usStartTime);
void OS_StartAlarm(OS_ALARM* pAlarm);
void OS_StartAbsAlarm(OS_ALARM* pAlarm, OS_ALARM_TICK * pRawAbsRef);

void OS_SetAlarmPeriod_(OS_ALARM* pAlarm, OS_U32 usPeriod);
void OS_SetAlarmOffset_(OS_ALARM* pAlarm, OS_U32 usOffset);
OS_BOOL OS_IsAlarm(OS_ALARM  *pt);

/**
\defgroup	OS_EXTENSION_ALM APIs
@{
  High resolution Alarm APIs.
\ingroup   	OS_EXTENSION_ALARMS
*/

/**
Must be called in _OS_SystemAlarmHandler (RTOS_Init.c)
*/
void OS_ALARM_Handler(void);


OS_ALARM_TICK OS_GetTickRef(void);

/**
Deletes an alarm
\param  [in]    pAlarm pointer to alarm (not modifiable)

\par Preconditions:
\li alarm was created by OS_CreateAlarm()
*/
void OS_DeleteAlarm(OS_ALARM* pAlarm);
...
\li alarm was created by OS_CreateAlarm()
*/
void OS_StopAlarm(OS_ALARM* pAlarm);

/**
Sets the Activation count property of an alarm
\param  [in]    pAlarm pointer to alarm (not modifiable)
\param  [in]    ActivationsCount number of activations
\par Preconditions:
\li alarm was created by OS_CreateAlarm()
*/
void OS_SetAlarmActivationsCount_(OS_ALARM* pAlarm, int ActivationsCount);
...
\hideinitializer
Creates an alarm.
The alarm is not automatically started. This has to be done explicitly by a call of OS_StartAlarm(), OS_STARTALARMSF_ONESHOT()
OS_STARTALARMSF()

\param  [in]    pAlarm pointer to alarm (not modifiable)
\param  [in]    Hook pointer to hook routine (not modifiable, checked if valid hook only in debug versions )
\param  [in]    usPeriod period in us (can be modified later). 
It defines the period of the subsequent expirations.
\param  [in]    usOffset offset in us (can be modified later). 
It is the time of the first expiration after the alarm is started.

\par Default alarm's configuration:
Alarm is infinite.

\par Assertions (debug version only):
\li Out of range: usOffset, usPeriod
...
*/
#define  OS_CreateAlarm(pAlarm, Hook, usPeriod, usOffset)\
OS_ASSERT_NOTOORALARMus(usOffset);\
OS_ASSERT_NOTOORALARMus(usPeriod);\
OS_CreateAlarm_( pAlarm,  Hook, usPeriod, usOffset)
...
\par Assertions (debug version only):
\li Out of range: if usPeriod not in range OS_Error is called with error code OS_ERR_OOR_US

\sa OS_SetAlarmPeriod_
...
*/
#define  OS_SetAlarmPeriod (pAlarm, usPeriod)\
OS_ASSERT_NOTOORALARMus(usPeriod);\
OS_SetAlarmPeriod_(pAlarm, usPeriod)
...
\param  [in]    pAlarm pointer to alarm (not modifiable)
\param  [in]    usOffset offset in us (can be modified later). 
It defines the offset of the first expiration.

\par Assertions (debug version only):
\li Out of range: if usOffset not in range OS_Error is called with error code OS_ERR_OOR_US

\sa OS_SetAlarmOffset_
...
\note		\par \b Note 1:
			documentation by Roberto Fioravanti
*/
#define  OS_SetAlarmOffset (pAlarm, usOffset)\
OS_ASSERT_NOTOORALARMus(usOffset);\
OS_SetAlarmOffset_(pAlarm, usOffset);


/**
\hideinitializer
Safely starts an alarm:
\li which was created by OS_CreateAlarm()
\li which was stopped by calling OS_StopAlarm().
\li or running (OS_StopAlarm is called)
\n in single shot mode.


\param  [in]    pAlarm pointer to alarm.
\param  [in]    usOffset offset in us. 
It defines the offset of the first expiration.

\par Assertions (debug version only):
\li Out of range: if usOffset not in range OS_Error is called with error code OS_ERR_OOR_US

\sa OS_StopAlarm, OS_SetAlarmPeriod_, OS_SetAlarmOffset_, OS_StartAlarm
...
\note		\par \b Note 1:
			documentation by Roberto Fioravanti
*/
#define OS_STARTALARMSF_ONESHOT(pAlarm, usOffset_)    \
OS_IncDI();\
OS_StopAlarm(pAlarm);                                 \
OS_ASSERT_NOTOORALARMus(usOffset_);                    \
OS_SetAlarmPeriod_(pAlarm, 0);                  \
OS_SetAlarmOffset_(pAlarm, usOffset_);                  \
OS_StartAlarm(pAlarm);\
OS_DecRI()
...
\note		\par \b Note 1:
			documentation by Roberto Fioravanti
*/
#define OS_START_ABSALARM_ONESHOT(pAlarm, usOffset_, pRef)    \
OS_IncDI();\
OS_StopAlarm(pAlarm);                                 \
OS_ASSERT_NOTOORALARMus(usOffset_);                    \
OS_SetAlarmPeriod_(pAlarm, 0);                  \
OS_SetAlarmOffset_(pAlarm, usOffset_);                  \
OS_StartAbsAlarm(pAlarm, pRef);\
OS_DecRI()
...
\param  [in]    usOffset offset in us . 
It defines the offset of the first expiration.
\param  [in]    usPeriod period in us. Period of the subsequent expirations.
\param  [in]    count number of reactivations


\par Assertions (debug version only):
\li Out of range: if usOffset or usPeriod not in range OS_Error is called with error code OS_ERR_OOR_US
...
\note		\par \b Note 1:
			documentation by Roberto Fioravanti
*/
#define OS_STARTALARMSF(pAlarm, us_Offset, us_Period,  count)    \
OS_IncDI();\
OS_StopAlarm(pAlarm);                               \
OS_ASSERT_NOTOORALARMus(us_Offset);                 \
OS_ASSERT_NOTOORALARMus(us_Period);                 \
OS_SetAlarmOffset_(pAlarm, us_Offset);              \
OS_SetAlarmPeriod_(pAlarm, us_Period);              \
OS_SetAlarmActivationsCount_(pAlarm, count);        \
OS_StartAlarm(pAlarm);                              \
OS_DecRI()
...
\note		\par \b Note 1:
			documentation by Roberto Fioravanti
*/
#define OS_START_ABSALARM(pAlarm, us_Offset, us_Period,  count, pRef)    \
OS_IncDI();\
OS_StopAlarm(pAlarm);                               \
OS_ASSERT_NOTOORALARMus(us_Offset);                 \
OS_ASSERT_NOTOORALARMus(us_Period);                 \
...
/**
\defgroup	OS_EXTENSION_CLEAN_API APIs
@{
  Clean APIs.
\ingroup   	OS_EXTENSION_CLEAN
*/

/**
Must be called in \todo
*/
void OS_Clean(void);
