...
void OS_Error(int ErrCode) {
  ...
  OS_EI();
  OS_ErrorCode = ErrCode;
  Tools_ProcessOSErrorStack(OS_ErrorCode);

  PLT_ERROR_FATAL(PLT_ERROR_OS_1);
...
/*
 When this happens, error has occured.
...
#define OS_ALARM_SERVICE_ID_START                             0
#define OS_ALARM_ERROR_ID_START                               0

    1.1 The following are the current services ID:
#define OS_StartAlarm_ServiceId                               (OS_ALARM_SERVICE_ID_START + 1)
#define OS_SetAlarmActivationsCount_ServiceId                 (OS_ALARM_SERVICE_ID_START + 2)
#define OS_SetAlarmOffset_ServiceId                           (OS_ALARM_SERVICE_ID_START + 3)
#define OS_SetAlarmPeriod_ServiceId                           (OS_ALARM_SERVICE_ID_START + 4)

    1.2 The following are the current errorcodes:
#define OS_AlarmActive_ErrorId                                (OS_ALARM_ERROR_ID_START + 1)
 
*/
unsigned char OS_X_Status=0;
