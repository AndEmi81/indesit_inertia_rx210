...
bool_t prsIsPressSwitchPres = FALSE;
...
\history
\date 01/04/2014
\n Added a check for the presence of the Linear Pressure Switch on the SF
...
void PM_SFT_WM_PressSwitch_Init(sft_module_status_t *pressSwitchMod)
{
	...
	uint8_t lliVal = PM_SFT_WM_IsLLIPresent(PRESSLIN_I);
	/* If the Linear Pressure Switch is not defined you must return */
	SAFETY_REGION_START();

	if(lliVal == TRUE)
	{
		prsIsPressSwitchPres = TRUE;
	}
	else if(lliVal == COMMON_BLOADER_ERR)	/* Error */
	{
		err = BAD_PARAMETERS_SF;
	}
	else
	{
		/* No linear pressure switch - module disabled */
	}

	SAFETY_DATA_UPDATE(prsIsPressSwitchPres);
	SAFETY_REGION_END();
...
\history
\date 01/04/2014
\n Added a check on the presence of the Linear Pressure switch on SF.
\n If the Pressure Switch is not defined, the function do nothing and
\n simply returns
*/
void PM_SFT_WM_PressSwitch_Monitor(iom_loads_struct_t *newLoads, sft_module_status_t *prsSwitchMod, uint8_t applReqFlags)
{
	...
	SAFETY_DATA_CHECK(prsIsPressSwitchPres);
	if(prsIsPressSwitchPres == FALSE)
	{
