...
enum eSFT_WrSftEvent
{
	...
	EVENT_WM_WR_PIN_CHECK,
	EVENT_WM_WR_RST_PINCHK,
	EVENT_WM_WR_RESTART_PINCHK
};
...
#if defined(CONFIG_DIGITAL_OUTPUT_MULTI_SELECTABLE_PINS)
	if (sftWRstatus.loadPos != LOAD_NOT_FOUND)
	{
		if (LIB_IOM_GetLoadPilotType(sftWRstatus.loadPos) != DIGITAL_OUTPUT_MULTI_L)
		{
			sftWRstatus.pinCheck = WR_PINCHECK_DISABLED;
		}
	}
#endif
...
SFT_wrEventBmpType event = 0;	/* clear bmp*/
...
#if defined(CONFIG_DIGITAL_OUTPUT_MULTI_SELECTABLE_PINS)
		if ((curConsistency & PM_CS_STATUS_MASK) == PM_CS_STATUS_OFF)
		{
			if (pNewLoads->loads_req[sftWRstatus.loadPos] != LOAD_REG_OFF)
			{
				sft_module_status_t tmpStatus;

				if (0 == LIB_SFT_GetSafetyModStatus(SFT_MOD_PRESS_SWITCH, &tmpStatus))
				{
					/* If Wash resistor activation is not prevented by pressure switch, start pin check */
					if ((tmpStatus.status < SFT_MODULE_PROTECTION)
							&& (LIB_SFT_GetSafetyActionExecBmp() == 0)
							&& (sftWRstatus.pinCheck == 0))
					{
						event |= (BFMASK(EVENT_WM_WR_PIN_CHECK,1));
					}
				}
			}
			else
			{
				event |= (BFMASK(EVENT_WM_WR_RST_PINCHK,1));
			}
		}
#endif
		...
		break;
 		...
 		case SFT_MODULE_WARNING:
			...
			if( event & (BFMASK(EVENT_WM_WR_FAULT_RESET,1)) )
			{
				...
				sftWRstatus.faultCnt=0;
#if defined(CONFIG_DIGITAL_OUTPUT_MULTI_SELECTABLE_PINS)
				event |= BFMASK(EVENT_WM_WR_RESTART_PINCHK,1);
#endif
	...
	/* Reset pin check routine */
	if (event & (BFMASK(EVENT_WM_WR_RST_PINCHK,1)))
	{
		sftWRstatus.pinCheck = 0;
		if(safetyData->status < SFT_MODULE_WARNING )
		{
	...
	/* Start pin check routine */
	else if (event & (BFMASK(EVENT_WM_WR_PIN_CHECK,1)))
	{
		sftWRstatus.pinCheck = WR_PINCHECK_START;
		sftWRstatus.faultCnt = SFT_SINGLEPIN_CHK_TIMEOUT_100MS;
	}

	if ( (sftWRstatus.pinCheck != WR_PINCHECK_DISABLED) && (sftWRstatus.pinCheck))
	{
		uint8_t newOvr = LOAD_NOT_FOUND;

		if (safetyData->status <= SFT_MODULE_MONITORING)
		{
			/* Restart pin check step counter */
			if (event & (BFMASK(EVENT_WM_WR_RESTART_PINCHK,1)))
			{
				sftWRstatus.faultCnt = SFT_SINGLEPIN_CHK_TIMEOUT_100MS;
			}

			/* Timeout - go to pin check next step */
			if (!sftWRstatus.faultCnt)
			{
				sftWRstatus.pinCheck--;
				if (sftWRstatus.pinCheck)
				{
					sftWRstatus.faultCnt = SFT_SINGLEPIN_CHK_TIMEOUT_100MS;
				}
			}
			...
			if (sftWRstatus.pinCheck)
			{
				/* Notify pin check as monitoring of Short circuit failure */
				safetyData->faultSrc = FAULTSRC_WR_OFF_ERR_RELAY_CC;
				safetyData->status = SFT_MODULE_MONITORING;
			}
		}

		switch(sftWRstatus.pinCheck)
		{
		case 3:
			newOvr = LOAD_CFG_MR_MULTI_1 | LOAD_REG_OFF;
			break;
		case 2:
			newOvr = LOAD_REG_OFF;
			break;
		case 1:
			newOvr = LOAD_CFG_MR_MULTI_2 | LOAD_REG_OFF;
			break;
		default:
			break;
		}
		...
		{
			pNewLoads->loads_ovr[sftWRstatus.loadPos] = newOvr;
		}
	}
