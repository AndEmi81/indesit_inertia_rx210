...
#if defined(CONFIG_PM_MOTORGATE)
/**
@name macros to extract Motor Fault Param
@{
*/
/**
...
/**
returns phsae angle threshold [us]
*/
...
enum eSFT_MotorSftEvent
{
	...
	SFT_MOTOR_EVT_DRUM_STOP,
	/*slow down */
	SFT_MOTOR_EVT_SLOW_DOWN_CHECK_KO,					/**/
	SFT_MOTOR_EVT_TACH_OUT_OF_RANGE,
	SFT_MOTOR_EVT_TACH_SLOW_DOWN_READY,
	SFT_MOTOR_EVT_TACH_IN_RANGE,
	SFT_MOTOR_EVT_RESET_PREFAULT_REQ,
	/* actions*/
	SFT_MOTOR_EVT_TACHKO_MOTOR_PROT_REQ,
	/*fbk*/
	SFT_MOTOR_EVT_FBK_ERR_INIT,
	SFT_MOTOR_EVT_FBK_GEN_ERR,
	SFT_MOTOR_EVT_FBK_RT_SC_ERR,
	SFT_MOTOR_EVT_FBK_T_DM_OCCUR,
	SFT_MOTOR_EVT_FBK_T_DM_ERR,
	SFT_MOTOR_EVT_FBK_T_DM_PROC_ACTIVE,
	SFT_MOTOR_EVT_FBKKO_MOTOR_PROT_REQ,
	SFT_MOTOR_EVT_DM_PROC_PTC_OPEN_REQ,
	SFT_MOTOR_EVT_REL_FAULT_INIT,
	SFT_MOTOR_EVT_REL_FAULT,
	SFT_MOTOR_EVT_REL_MOTOR_PREFAULT_REQ,
	SFT_MOTOR_EVT_MOTOR_OVR_OFF,						/* not used */
...
#endif
	/* Common */
	PM_SFT_WM_MOTOR_NOZC_WHILE_RUNNING,						/**< bit, unexpected no ZC while motor running if set implies also SFT_MOTOR_EVT_NO_ZC is set */
	/* AC motor */
	PM_SFT_WM_MOTOR_TACH_READY,								/**< bit, tacho ready  */
	PM_SFT_WM_MOTOR_TACH_VIRTUAL_STOP,						/**< bit, wait 4 stop missing tacho */
	PM_SFT_WM_MOTOR_NOTACH_SSTART,							/**< bit, no tach @ soft start  */
	PM_SFT_WM_WAIT_FOR_MOTOR_STOP,							/**< bit, wait 4 stop with tach OK */
	PM_SFT_WM_MOTOR_FORCE_FBK_PROT,							/**< bit,  */
	PM_SFT_WM_MOTOR_FORCE_DM_PROT,
	/*num of states */
...
{
	unsigned char fbkPrefaultTime1;				/**< FB Based Prefault Filter time [100 ms], 2 bytes unsigned , BE	*/
...
{
	unsigned char speed1;				/**< FB Based Prefault Filter time [100 ms], 2 bytes unsigned , BE	*/
	unsigned char speed0;
}MotorCmdType;
/**
debug struct
...
typedef enum ePM_SFT_WM_FSM_DM
{
	PM_SFT_WM_FSM_DM_INIT = 0,
	PM_SFT_WM_FSM_DM_IDLE = 1, /*  detection */
	PM_SFT_WM_FSM_DM_WAIT_4_TRIAC_MD_VALIDATED =2
}PM_SFT_WM_FSM_DM_Type;


typedef enum ePM_SFT_WM_FSM_DM_TRIAC_PROC
{
	PM_SFT_WM_FSM_DM_PROC_IDLE = 0,				/* init detection */
	PM_SFT_WM_FSM_DM_PROC_INIT,
	PM_SFT_WM_FSM_DM_PROC_BP_PTC_STEP
}PM_SFT_WM_FSM_DM_TriacProcType;

typedef struct PM_SFT_WM_FSM_Consistency_s{
	PM_SFT_WM_FSMConsistencyType	fsm_state;
	PM_SFT_WM_FSM_DM_Type			fsm_state_dm;
	PM_SFT_WM_FSM_DM_TriacProcType	fsm_state_dm_proc;
	SFT_MotorEventBmpType			evt;
	TOOLS_TIMER Tout;
	TOOLS_TIMER dmTout;
	TOOLS_TIMER dmToutProc;
	int validationTime;
	int TimeOut;
...
/* Functions Prototype */
/* ******************* */
static bool_t PM_SFT_WM_Is3phMotor(void);
/* Exported function */
...
static SFT_MotorEventBmpType PM_SFT_WM_FbkOKFSM(SFT_MotorEventBmpType evt, uint8_t consistency);
static SFT_MotorEventBmpType PM_SFT_WM_FbkGenericErrFSM(SFT_MotorEventBmpType evt);
static void PM_SFT_WM_FbkTriacDMFSMResetMgmt(SFT_MotorEventBmpType evt);
static SFT_MotorEventBmpType PM_SFT_WM_FbkTriacDMErrFSM(SFT_MotorEventBmpType evt, uint8_t consistency);
static SFT_MotorEventBmpType PM_SFT_WM_FbkTriacDMProcFSM(SFT_MotorEventBmpType evt);
static SFT_MotorEventBmpType PM_SFT_WM_FbkRelTriacSCErrFSM(SFT_MotorEventBmpType evt);
static void PM_SFT_WM_FbkTriacDMFSM_ProcSetRunning(bool_t bRun);
...
/**
\n This function initializes the status of the washing probe and
\n the state machine related to the safety of motor module.
...
void PM_SFT_WM_Motor_Monitor(iom_loads_struct_t *pNewLoads, sft_module_status_t *pSafetyStatus, uint8_t applReqFlags)
{
	...
	{
		event |= BFMASK(SFT_MOTOR_EVT_ZC_GOOD,1);
	}
	else
	{
	...
	if(!(event & BFMASK(SFT_MOTOR_EVT_MOTOR_REQ_ON,1)))
	{
		PM_UM_OverrideTriacSuspend(FALSE);
	}
	...
	{
		/* start speedFilter timer to filter 0 rpm */
		Tools_StartTimer(&(MotorData.speedFilter));
	}

#if defined(CONFIG_PM_MOTORGATE)
	if(event & BFMASK(SFT_MOTOR_EVT_DM_PROC_PTC_OPEN_REQ,1))
	{
		uint8_t doorLockPos = LIB_IOM_GetLoadPos((uint8_t) LD_DOOR_LOCK_POS);
		...
		if(doorLockPos != LOAD_NOT_FOUND)
		{
			...
			pNewLoads->loads_ovr[doorLockPos] = LOAD_REG_OFF;
		}
	}
#endif /* defined(CONFIG_PM_MOTORGATE) */
	...
	switch (pSafetyStatus->status)
	{
		...
		case SFT_MODULE_PROTECTION:
		{
			...
			if(!MotorData.bIsMotor3ph)
			{
				...
				if(event & (BFMASK(SFT_MOTOR_EVT_TACHKO_MOTOR_PROT_REQ,1)|BFMASK(SFT_MOTOR_EVT_MISSING_TACH,1)))
				{
					...
					else
					{
						...
						if(!(MotorData.state & BFMASK(PM_SFT_WM_MOTOR_TACH_VIRTUAL_STOP,1)) && (event & BFMASK(SFT_MOTOR_EVT_FBKKO_MOTOR_PROT_REQ,1)))
						{
							MotorData.state |= BFMASK(PM_SFT_WM_MOTOR_TACH_VIRTUAL_STOP,1);
							Tools_StartTimer(&(MotorData.timerVirtualStop));
							PM_SFT_WM_FSM_TachMonitorResetFSM();
			...
			if(!MotorData.bIsMotor3ph)
			{
				SFT_MotorEventBmpType evt=0;
				evt = (event & (BFMASK(SFT_MOTOR_EVT_MOTOR_REQ_ON,1)|BFMASK(SFT_MOTOR_EVT_FBKKO_MOTOR_PROT_REQ,1)));
		...
		case SFT_MODULE_RESET:
		{
			 ...
			 * TODO AC-Motor: to be defined when timer is to be restarted
			*/
...
#if defined(CONFIG_PM_MOTORGATE)
			/* TODO evaluate if it OK for 3ph motor too */
...
#if defined(CONFIG_PM_MOTORGATE)
			if((event & BFMASK(SFT_MOTOR_EVT_REL_FAULT,1)) == 0)
			{
				pSafetyStatus->status = SFT_MODULE_IDLE;
				pSafetyStatus->faultSrc = SFT_NO_FAULT_SRC;
...
static bool_t PM_SFT_WM_Is3phMotor(void)
{
	return (bool_t)(MotorData.bIsMotor3ph==TRUE);
}
...
static bool_t PM_SFT_WM_SwitchToProtection(SFT_MotorEventBmpType event)
{
	...
	else if(!(MotorData.state & BFMASK(PM_SFT_WM_MOTOR_FORCE_FBK_PROT,1)) && ((event & BFMASK(SFT_MOTOR_EVT_TACHKO_MOTOR_PROT_REQ,1)) || (MotorData.state & BFMASK(PM_SFT_WM_MOTOR_TACH_VIRTUAL_STOP,1))))
	{
		bFault = TRUE;
...
static SFT_MotorEventBmpType PM_SFT_WM_FSM_ACMotorMonitor(SFT_MotorEventBmpType evt)
{
	...
	if (MotorData.MotorPos != LOAD_NOT_FOUND )
	{
		...
		outevt |= PM_SFT_WM_FbkTriacDMProcFSM(evt|outevt); /* no event taken into account => TODO evt|outevt not needed at all */
		...
		else
		{
			...
			/* nothing to do */
			PM_SFT_WM_ConsistencyFSM_Resync();
			PM_SFT_WM_FbkTriacDMFSM_ProcSetRunning(FALSE);
		}
...
static SFT_MotorEventBmpType PM_SFT_WM_SoftStartFSM(SFT_MotorEventBmpType evt)
{
	...
	switch(PM_SFT_WM_Motor_SoftStart.fsm_state)
	{
		...
		case PM_SFT_WM_FSM_SOFTSTART_MOTOR_OFF_PAUSE:
			...
			{
				/* TODO : NO_ZC_WHILE_RUNNING not taken into account */
				Tools_DisableTimeout(&(PM_SFT_WM_Motor_SoftStart.Tout));
				PM_SFT_WM_Motor_SoftStart.fsm_state = PM_SFT_WM_FSM_SOFTSTART_WAIT_4_NOTACH;
				if( evt & BFMASK(SFT_MOTOR_EVT_MISSING_TACH_AT_SOFTSTART,1) )
				{
...
static SFT_MotorEventBmpType PM_SFT_WM_ConsistencyFSM(SFT_MotorEventBmpType evt, uint8_t u8Consistency)
{
	...
	switch(PM_SFT_WM_Motor_Consistency.fsm_state)
	{
		case PM_SFT_WM_FSM_CONSISTENCY_INIT:
			...
			PM_SFT_WM_Motor_Consistency.fsm_state_dm=PM_SFT_WM_FSM_DM_INIT;
			PM_SFT_WM_FbkTriacDMFSM_ProcSetRunning(FALSE);
			MotorData.state &= ~BFMASK(PM_SFT_WM_MOTOR_FORCE_DM_PROT,1);
			/*break;*/
			/* -fallthrough */
		case PM_SFT_WM_FSM_CONSISTENCY_WAIT_4_ZC_GOOD:
			...
			if(evt & BFMASK(SFT_MOTOR_EVT_ZC_GOOD,1))
			{
				...
				{
					case PM_CS_VALUE_OK:
						tmp |= PM_SFT_WM_FbkOKFSM(preEvt, u8Consistency);
						...
						if(!tmp)
						{
							...
							if(evt & (BFMASK(SFT_MOTOR_EVT_MOTOR_REQ_ON,1)))
							{
								PM_UM_OverrideTriacSuspend(FALSE);
							}
						}
					...
					case PM_CS_VALUE_ERR_DM:
						if(PM_SFT_WM_ConsistencyEnable())
						{
							tmp |= PM_SFT_WM_FbkTriacDMErrFSM(preEvt, u8Consistency);
							if(tmp & BFMASK(SFT_MOTOR_EVT_FBK_T_DM_ERR,1))
							{
								MotorData.state |= BFMASK(PM_SFT_WM_MOTOR_FORCE_DM_PROT,1);
							}
						}
				...
				if(! (MotorData.state & BFMASK(PM_SFT_WM_WAIT_FOR_MOTOR_STOP,1)))
				{
					...
					if( /* feedback or triac in ShortCircuit OR*/
						(tmp & (BFMASK(SFT_MOTOR_EVT_FBK_GEN_ERR,1)|BFMASK(SFT_MOTOR_EVT_FBK_RT_SC_ERR,1)) )
							||
						/* DiodeMode already detected */
						((MotorData.state & BFMASK(PM_SFT_WM_MOTOR_FORCE_DM_PROT,1)) && (tmp & BFMASK(SFT_MOTOR_EVT_FBK_T_DM_ERR,1))))
					{
						...
						/*
						 * suspend triac activation
						*/
						if(evt & (BFMASK(SFT_MOTOR_EVT_MOTOR_REQ_ON,1)))
						{
							PM_UM_OverrideTriacSuspend(TRUE);
						}

						if( Tools_IsTimerElapsed(&(PM_SFT_WM_Motor_Consistency.Tout), PM_SFT_WM_Motor_Consistency.TimeOut) &&
								/* doorlock request on  || module not in idle (e.g. in monitoring)
								 doorlock request off prevents the request of protection in order to allow unlocking door
								*/
								(
									(evt & BFMASK(SFT_MOTOR_EVT_LOCK_DOOR_REQ,1)) ||
									((evt & BFMASK(SFT_MOTOR_EVT_MONIT,1)) && (evt & (BFMASK(SFT_MOTOR_EVT_MOTOR_REQ_ON,1))))
								)
							)
						{
					...
					else if( tmp & (BFMASK(SFT_MOTOR_EVT_REL_FAULT,1)) )
					{
						if(
							(Tools_IsTimerElapsed(&(PM_SFT_WM_Motor_Consistency.Tout), PM_SFT_WM_Motor_Consistency.TimeOut) ) 	&&
								/*
									doorlock request on  || if module not in idle (e.g. in monitoring the
									doorlock request is off to prevent the request of the protection in order to allow unlocking door
								*/
								(
									(evt & BFMASK(SFT_MOTOR_EVT_LOCK_DOOR_REQ,1)) ||
									((evt & BFMASK(SFT_MOTOR_EVT_MONIT,1)) && (evt & (BFMASK(SFT_MOTOR_EVT_MOTOR_REQ_ON,1))))
					...
					if( 	/* Diode mode for the first time*/
							((tmp & BFMASK(SFT_MOTOR_EVT_FBK_T_DM_ERR,1)) && !(MotorData.state & BFMASK(PM_SFT_WM_MOTOR_FORCE_DM_PROT,1)))
							&& (evt & BFMASK(SFT_MOTOR_EVT_LOCK_DOOR_REQ,1) ))
					{
						tmp |= BFMASK(SFT_MOTOR_EVT_FBKKO_MOTOR_PROT_REQ,1);
					}
...
/**
 ...
 SFT_MOTOR_EVT_REL_FAULT
 */
static SFT_MotorEventBmpType PM_SFT_WM_FbkOKFSM(SFT_MotorEventBmpType evt, uint8_t u8Consistency)
{
	SFT_MotorEventBmpType out=0;
	uint8_t doorLockPos = 0;
	doorLockPos = LIB_IOM_GetLoadPos((uint8_t) LD_DOOR_LOCK_POS);

	if(BLOCCO_PORTA_CERA == PM_DoorLock_GetDoorLockType())
	{
		if((u8Consistency == (PM_CS_VALUE_OK|PM_CS_STATUS_OFF)) &&

				((Tools_IsTimerElapsed(&(MotorData.speedFilter), 3000)) && (MotorData.iDrumSpeed == 0)) &&
				(LOAD_REG_ON == LIB_IOM_GetLoadStatus(doorLockPos)))
		{
			out |= BFMASK(SFT_MOTOR_EVT_REL_FAULT,1);
		}
	}
	return out;
}
...
static SFT_MotorEventBmpType PM_SFT_WM_FbkGenericErrFSM(SFT_MotorEventBmpType evt)
{
	...
	if(!(MotorData.state & BFMASK(PM_SFT_WM_MOTOR_FORCE_DM_PROT,1)))
	{
		out |= BFMASK(SFT_MOTOR_EVT_FBK_GEN_ERR,1);
	}
	else
	{
		out |= BFMASK(SFT_MOTOR_EVT_FBK_T_DM_ERR,1);
	}
...
/**
controls DM Err detection fsm, reset the state if no occurrence of DM (SFT_MOTOR_EVT_FBK_T_DM_OCCUR)
\param [in]	evt events (SFT_MotorEventBmpType) (SFT_MOTOR_EVT_TACH_IN_RANGE, SFT_MOTOR_EVT_SLOW_DOWN_CHECK_KO)
*/
static void PM_SFT_WM_FbkTriacDMFSMResetMgmt(SFT_MotorEventBmpType evt)
{
	if((evt & BFMASK(SFT_MOTOR_EVT_FBK_T_DM_OCCUR,1) ) == 0 )
	{
		/* */
		PM_SFT_WM_Motor_Consistency.fsm_state_dm=PM_SFT_WM_FSM_DM_INIT;
	}
...
/**
DM Err detection fsm
INIT: initialization
IDLE: detection and validation (VALIDATION_TIME x 100 ms)
	- drum speed @ 0 or not in range: validation requires DM detection procedure
	- drum speed in range: validation by time

\param [in]	evt events (SFT_MotorEventBmpType) (SFT_MOTOR_EVT_TACH_IN_RANGE, SFT_MOTOR_EVT_SLOW_DOWN_CHECK_KO)
\param [in]	consistency consistency

\retval SFT_MotorEventBmpType,  bitmap of ( SFT_MOTOR_EVT_FBK_T_DM_OCCUR, SFT_MOTOR_EVT_FBK_T_DM_ERR, SFT_MOTOR_EVT_FBK_T_DM_PROC_ACTIVE)
\note precondition
 SFT_MOTOR_EVT_ZC_GOOD is set in evt
 */
static SFT_MotorEventBmpType PM_SFT_WM_FbkTriacDMErrFSM(SFT_MotorEventBmpType evt, uint8_t consistency)
{
	SFT_MotorEventBmpType out=0;

	switch(PM_SFT_WM_Motor_Consistency.fsm_state_dm)
	{
		case PM_SFT_WM_FSM_DM_INIT:
			PM_SFT_WM_Motor_Consistency.fsm_state_dm++;
			PM_SFT_WM_FSM_SlowDownCheckResync();
			PM_SFT_WM_FbkTriacDMFSM_ProcSetRunning(FALSE);
			out |= BFMASK(SFT_MOTOR_EVT_FBK_T_DM_OCCUR,1);
			Tools_StartTimer(&(PM_SFT_WM_Motor_Consistency.dmTout));
			PM_SFT_WM_Motor_Consistency.validationTime = 100*PM_SFT_WM_GET_FBK_VALIDATION_TIME((PM_SFT_WM_SFMotorFaultType*)MotorData.faultParam.Data);
		break;
		case PM_SFT_WM_FSM_DM_IDLE:
			if(Tools_IsTimerElapsed(&(PM_SFT_WM_Motor_Consistency.dmTout), PM_SFT_WM_Motor_Consistency.validationTime))
			{
					if( !(evt & BFMASK(SFT_MOTOR_EVT_TACH_IN_RANGE,1)) || ((Tools_IsTimerElapsed(&(MotorData.speedFilter), 3000)) && (MotorData.iDrumSpeed == 0)))
					{
						if((consistency & PM_CS_STATUS_ON))
						{
							PM_SFT_WM_FbkTriacDMFSM_ProcSetRunning(TRUE);
							PM_SFT_WM_Motor_Consistency.fsm_state_dm = PM_SFT_WM_FSM_DM_WAIT_4_TRIAC_MD_VALIDATED;
							out |= BFMASK(SFT_MOTOR_EVT_FBK_T_DM_OCCUR,1);
						}
						else
						{
					...
					else if( evt & BFMASK(SFT_MOTOR_EVT_SLOW_DOWN_CHECK_KO,1) )
					{
						PM_SFT_WM_Motor_Consistency.fsm_state_dm = PM_SFT_WM_FSM_DM_WAIT_4_TRIAC_MD_VALIDATED;
						out |= BFMASK(SFT_MOTOR_EVT_FBK_T_DM_OCCUR,1);
					}
		...
		case PM_SFT_WM_FSM_DM_WAIT_4_TRIAC_MD_VALIDATED:
		{

			if((PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc == PM_SFT_WM_FSM_DM_PROC_BP_PTC_STEP) &&
					!(evt & BFMASK(SFT_MOTOR_EVT_DM_PROC_PTC_OPEN_REQ,1)))
			{
				out |= BFMASK(SFT_MOTOR_EVT_FBK_T_DM_ERR,1)|BFMASK(SFT_MOTOR_EVT_FBK_T_DM_OCCUR,1)|BFMASK(SFT_MOTOR_EVT_FBK_T_DM_PROC_ACTIVE,1);
			}
			else if((PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc == PM_SFT_WM_FSM_DM_PROC_IDLE))
			{
				out |= BFMASK(SFT_MOTOR_EVT_FBK_T_DM_ERR,1)|BFMASK(SFT_MOTOR_EVT_FBK_T_DM_OCCUR,1);
			}
			else
			{
				/* do nothing */
...
/* DM proc */
/**
Controls DM detection procedure PM_SFT_WM_FbkTriacDMFSM_ProcSetRunning.

\param [in]	bRun desired state of FSM DM detection procedure, only for WAX doorlock.
If TRUE DM detection procedure is activated, if FALSE back to IDLE.
*/
static void PM_SFT_WM_FbkTriacDMFSM_ProcSetRunning(bool_t bRun)
{
	if(BLOCCO_PORTA_CERA == PM_DoorLock_GetDoorLockType())
	{
		if((PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc == PM_SFT_WM_FSM_DM_PROC_IDLE) && bRun)
		{
			PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc = PM_SFT_WM_FSM_DM_PROC_INIT;
		}
		else if ((PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc >= PM_SFT_WM_FSM_DM_PROC_INIT) && !bRun)
		{
			PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc = PM_SFT_WM_FSM_DM_PROC_IDLE;
		}
	}
}

/**
DM detection procedure PM_SFT_WM_FbkTriacDMProcFSM
IDLE: doing nothing
INIT: initialization
STEP: core
Duration: 2 x DOOR_LOCK_OFF_TIME from faultParam structure (x 100 ms)
Form 0 to DOOR_LOCK_OFF_TIME WAX doorlock is not driven (override) to detect if motor
thermal overload protection is active.
From     DOOR_LOCK_OFF_TIME to 2 x DOOR_LOCK_OFF_TIME WAX doorlock is not overridden
then back to IDLE

\param [in]	evt events (SFT_MotorEventBmpType)
\return SFT_MotorEventBmpType
\retval SFT_MOTOR_EVT_DM_PROC_PTC_OPEN_REQ
\attention global variable affected PM_SFT_WM_Motor_Consistency
 */
static SFT_MotorEventBmpType PM_SFT_WM_FbkTriacDMProcFSM(SFT_MotorEventBmpType evt)
{
	SFT_MotorEventBmpType out=0;
	...
	switch(PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc)
	{
		case PM_SFT_WM_FSM_DM_PROC_IDLE:
			break;


		case PM_SFT_WM_FSM_DM_PROC_INIT:
			PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc++;
			Tools_StartTimer(&(PM_SFT_WM_Motor_Consistency.dmToutProc));
			out |= BFMASK(SFT_MOTOR_EVT_DM_PROC_PTC_OPEN_REQ,1);
			/* -fallthrough */
		case PM_SFT_WM_FSM_DM_PROC_BP_PTC_STEP:

			timeout = 100* PM_SFT_WM_GET_DOOR_LOCK_OFF_TIME((PM_SFT_WM_SFMotorFaultType*)MotorData.faultParam.Data);

			if(Tools_IsTimerElapsed(&(PM_SFT_WM_Motor_Consistency.dmToutProc), (OS_TIME)2*timeout))
			{
				PM_SFT_WM_Motor_Consistency.fsm_state_dm_proc = PM_SFT_WM_FSM_DM_PROC_IDLE;
			}else if(!Tools_IsTimerElapsed(&(PM_SFT_WM_Motor_Consistency.dmToutProc), (OS_TIME)timeout))
			{
				out |= BFMASK(SFT_MOTOR_EVT_DM_PROC_PTC_OPEN_REQ,1);
			}
			else
...
static void PM_SFT_WM_Motor_InitState(void)
{
	...
	MotorData.iDrumSpeed=0;
	MotorData.bIsMotor3ph=LIB_IOM_ExtdLoadIsExternal(EXTLD_ENGINE);
...
#endif
#if defined(CONFIG_EXTERN_MODULE_L_SUPPORT)
	if (MotorData.commStep == PM_SFT_WM_MOTOR_COMMSUSPEND_COMM)
	{
...
#endif
	if(pStatus->status == SFT_MODULE_MONITORING)
	{
