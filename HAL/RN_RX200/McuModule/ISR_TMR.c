...
#if defined(TCIV3_IRQ_SUPPORT)
int TCIV3_ISR(void);
__attribute__((weak)) int TCIV3_ISR(void)
{
	if( TMR_GetClearSourceId(HAL_TMR_MTU3) == TCNT_CLEARING_OVERFLOW ){
		HWCNTR_ISR(HW_TMR_ID_3);
		return 1;
	}
    return 0;
...
#if defined(TCIV4_IRQ_SUPPORT)
int TCIV4_ISR(void);
__attribute__((weak)) int TCIV4_ISR(void)
{
	if( TMR_GetClearSourceId(HAL_TMR_MTU4) == TCNT_CLEARING_OVERFLOW ){
		HWCNTR_ISR(HW_TMR_ID_4);
		return 1;
	}
    return 0;
