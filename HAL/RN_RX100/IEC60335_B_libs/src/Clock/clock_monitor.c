...
#define CAC_REFERENCE_DIVISOR	128
#define CAC_CLOCK_DIVISOR		32
	#define CLOCK_COUNT_EXPECTED (((BUS_FREQUENCY_Hz/CAC_CLOCK_DIVISOR)/IWDTCLK_FREQ_Hz)* (CAC_REFERENCE_DIVISOR))

#if (CLOCK_COUNT_EXPECTED > 0xFFFF )
#error "BAD CAC configuration"
#endif
#endif
...
Return value  : 
***********************************************************************************/
#define IRQ_CAC_FERRF	32
#define IRQ_CAC_MENDF	33
#define IRQ_CAC_OVFF	34
#ifndef CLOCK_MONITOR_USE_EXTERNAL_REFERENCE_CLOCK
void ClockMonitor_Init(CLOCK_MONITOR_ERROR_CALL_BACK CallBack)
{
	...
	/*Clock select =  HOCO clock / 32*/
	CAC.CACR1.BYTE = 0x34;
	
#if (CAC_REFERENCE_DIVISOR == 1024)
	/*Reference clock - IWDTCLK / 1024 , Internal clock*/
	CAC.CACR2.BYTE = 0x29;
#elif (CAC_REFERENCE_DIVISOR == 128)
	/*Reference clock - IWDTCLK / 128 , Internal clock*/
	CAC.CACR2.BYTE = 0x19;
#else
#error "BAD CAC configuration"
#endif
	...
	ICU.IR[IRQ_CAC_OVFF].BIT.IR = 0;
	/*ICU Interrupt priority*/
	ICU.IPR[IRQ_CAC_OVFF].BIT.IPR = CLOCK_MONITOR_INTERRUPT_PRIORITY;
	/*Enable the interrupt with the ICU*/
	ICU.IER[0x04].BIT.IEN2 = 1;
	
	/*Frequency error (FERRF) ---------------------------------------*/
	/*Make sure IR flag is clear*/
	ICU.IR[IRQ_CAC_FERRF].BIT.IR = 0;
	/*ICU Interrupt priority*/
	ICU.IPR[IRQ_CAC_FERRF].BIT.IPR = CLOCK_MONITOR_INTERRUPT_PRIORITY;
	/*Enable the interrupt with the ICU*/
