...
**********************************************************************
*/
volatile int dbg_trace_1=0;
volatile int dbg_trace_2=0;
/*********************************************************************
*
...
#define ISPR_BASE_ADDR       (0x00087300)
#define IRER_BASE_ADDR       (0x00087200)
...
#define IPR05                (*(volatile EVENACCESS OS_U8*)  (0x00087305))
...
#define CMT0_BASE_ADDR       (0x00088000)
#define CMT0_MSTPCR          (MSTPCRA)
#define CMT0_MSTPCR_BIT      (15)

#define CMT0_VECT            28
#define CMT0_IPR             (IPR04)
#define CMT0_IER             (IER03)
#define CMT0_IER_BIT         (4)
#define OS_TIMER_INT_PRIO    (0x01)     /* Lowest priority */
#define OS_TIMER_VECT        CMT0_VECT
#define CMT_CMSTR0          (*(volatile EVENACCESS OS_U16*) (CMT0_BASE_ADDR + 0x00))

#define CMT0_CMCR            (*(volatile EVENACCESS OS_U16*) (CMT0_BASE_ADDR + 0x02))
#define CMT0_CMCNT           (*(volatile EVENACCESS OS_U16*) (CMT0_BASE_ADDR + 0x04))
#define CMT0_CMCOR           (*(volatile EVENACCESS OS_U16*) (CMT0_BASE_ADDR + 0x06))


#define CMT1_VECT            29
#define CMT1_IPR             (IPR05)
#define CMT1_IER             (IER03)
#define CMT1_IER_BIT         (5)

#define CMT1_CMCR            (*(volatile EVENACCESS OS_U16*) (CMT0_BASE_ADDR + 0x08))
#define CMT1_CMCNT           (*(volatile EVENACCESS OS_U16*) (CMT0_BASE_ADDR + 0x0A))
#define CMT1_CMCOR           (*(volatile EVENACCESS OS_U16*) (CMT0_BASE_ADDR + 0x0C))

#define SYS_TMR_MATCH_VAL	CMT0_CMCOR

#if defined(OS_SYSALARM_MTU1_A)
#define MTU1_DIV_1					0
#define MTU1_DIV_4					1
#define MTU1_DIV_16					2
#define MTU1_DIV_64					3

#define ALM_TMR_BASE_ADDR      		(0x00088780)
#define OS_ALM_TIMER_VECT       	(121)
#define OS_ALM_TIMER_PRESCALE   	(1)
#define OS_ALM_TIMER_INT_PRIO		(0x01)

#define ALM_TMR_MATCH_A          	(*(volatile EVENACCESS OS_U16*)(ALM_TMR_BASE_ADDR + 0x08 ))
#define ALM_TMR_MATCH_B          	(*(volatile EVENACCESS OS_U16*)(ALM_TMR_BASE_ADDR + 0x0A ))
#define ALM_TMR_TCNT           		(*(volatile EVENACCESS OS_U16*)(ALM_TMR_BASE_ADDR + 0x06 ))

#define OS_ALARM_HW_CNT           	ALM_TMR_TCNT

#define OS_ALARM_PRIO_INT_REG     	(*(volatile EVENACCESS OS_U8*) (ISPR_BASE_ADDR + OS_ALM_TIMER_VECT))
#define OS_ALARM_MATCH_ICU       	(*(volatile EVENACCESS OS_U8*) (IRER_BASE_ADDR + (OS_ALM_TIMER_VECT >> 3)))
#define OS_ALARM_MATCH_ICU_MSK   	(1uL << (OS_ALM_TIMER_VECT & 0x7))
#define OS_ALARM_HW_CNT_MATCH_VAL 	ALM_TMR_MATCH_A

#define OS_ALMCT_IR               	(*(OS_U8*)(IRR_BASE_ADDR + OS_ALM_TIMER_VECT))

#define OS_ALARM_STOP  {MTU.TSTR.BIT.CST1 = 0; MTU1.TIER.BIT.TGIEA = 0; }
#define OS_ALARM_START {MTU.TSTR.BIT.CST1 = 1; MTU1.TIER.BIT.TGIEA = 1; }
...
void OS_InitHW(void) {
  ...
  */
  ...
  PRCR = 0xA502;                                           		/* Protect off	- 210*/
   ...
   CMT0_MSTPCR &= ~(1uL << CMT0_MSTPCR_BIT);                /* Activate CMT unit */
   CMT_CMSTR0   = 0x00;                                     /* Stop counter */
   CMT0_CMCR   |= OS_SYSTEM_PS;
   CMT0_CMCR   |= (1uL << 6);                               /* Compare match interrupt enable */
   CMT0_CMCOR   = (OS_PCLK_TIMER/OS_PSSYS)/OS_TICK_FREQ;    /* Count value for 10 msec resolution */

   /* Enable CMT0 interrupt in interrupt controller */
   CMT0_IPR     = OS_TIMER_INT_PRIO;                        /* Set interrupt priority, lowest priority */
   CMT0_IER    |= (1uL << CMT0_IER_BIT);                    /* Enable CMT interrupt */
   *(OS_U8*)(IRR_BASE_ADDR + OS_TIMER_VECT)  = 0;           /* Clear interrupt request */
   *(OS_U8*)(ISEL_BASE_ADDR + OS_TIMER_VECT) = 0;           /* Select interrupt source CPU */

   /*  Start timer */
   CMT_CMSTR0  = 0x01;                                     /* Start counter */
#else
#error	"Unsupported System timer."
   ...
   MSTPCRA =  0; /* enabled the module clock */
   /* configure timer */
   OS_ALARM_STOP;

   do{
	   MTU.TSTR.BIT.CST1 = 0;	/* stop counter */
   }while(MTU.TSTR.BIT.CST1 != 0 );

   MTU1.TCR.BIT.CCLR = 0; 	/* disable  clearing */

   do{
	   MTU1.TCR.BYTE     = OS_SYSALARM_PS; 		/* prescaler 16 */
   }while(MTU1.TCR.BYTE != OS_SYSALARM_PS );
...
OS_U32 OS_ConvertCycles2us(OS_U32 Cycles) {
  Cycles *= OS_PSSYS;   /* Eliminate effect of fixed prescaler*/
...
OS_U32 OS_Convert_us2Cycles(OS_U32 us) {
	...
	Cycles = us * (0x20);
	Cycles = (unsigned int)Cycles/(unsigned int)(1000000/((OS_PCLK_TIMER/0x20)/OS_PSSYS));
	return Cycles;
}
/*********************************************************************
*
